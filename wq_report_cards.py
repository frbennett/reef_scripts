...
HG_URL='http://www.flowmatters.com.au/hg_202310'
...
class WQReportCardBuilder(object):
    def __init__(self,results_dir,data_dest,loads_file,concentrations_file,water_years=DEFAULT_WATER_YEARS,csv_files_have_header=True):
        ...
        self.mass_balance_builder = MassBalanceBuilder(results_dir,self.ds)
    ...
    def build(self):
        ...
        self.mass_balance_builder.build()
...
class MassBalanceBuilder(object):
    def __init__(self,results_dir,dataset):
        self.results_dir = results_dir
        self.ds = dataset
        gbr.init(results_dir)
        self.runs = gbr.available()

    def build(self):
        logging.info('Building mass balance summaries')
...
class AnnualComparisonBuilder(object):
    ...
    def build(self):
        logging.info('Building annual comparisons')
        self.initialise_obs()
        self.load_obs()
        self.load_runs()
